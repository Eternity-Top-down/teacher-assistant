...
- 个人风格样例：一对一反馈样例和晚辅反馈样例分开维护，支持新增、查看详情、编辑、启用/停用、删除；生成时只学习对应模块的语气、结构和详略，样例库可多保存，参与生成时建议挑 1-3 条高质量反馈，最多 5 条，支持从已保存反馈一键设为样例。
- 晚辅模块：维护晚辅班级、批量录入晚辅学生，按天、按周或按月生成和管理晚辅反馈；每条反馈可填写本次学科，用于生成“某学生某科作业完成情况”口径；生成时优先依据老师输入的作业完成度、作业质量、晚辅状态、主要问题和后续提醒，不足的信息不硬凑，并可按时间段、反馈类型和学生姓名查询全部晚辅反馈。
...
样例库可以多保存，方便积累不同场景的好文案；真正参与生成时，建议只启用 1-3 条自己最满意、最希望 AI 模仿的高质量真实反馈。建议先把样例中的姓名匿名化为“学生A / 学生B”，避免在样例库和 prompt 中暴露真实学生信息。
