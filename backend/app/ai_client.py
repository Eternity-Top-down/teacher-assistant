...
async def generate_evening_feedback(
    ...
    if style_text:
        style_rule = """
8. 本次已启用老师的晚辅反馈风格样例，样例格式优先级高于默认按天/周/月段落建议。
9. 必须优先复刻样例的整体排版习惯：段落数量、段落长短、起句方式、是否使用标题、结尾提醒方式和整体详略。
10. 如果样例是单个自然段，本次也尽量写成单个自然段；如果样例有固定标题或固定顺序，本次也沿用相同标题形式和顺序。
11. 只学习样例的格式和表达方式，禁止复用样例里的学生事实、原句、题目、时间、具体表现、作业内容或问题细节。
12. 贴近样例格式时仍必须遵守事实边界：没有输入依据的内容不要补，不要为了凑齐样例里的某一项而编造。
""".strip()
        length_rule = "输出长短：以老师晚辅样例的篇幅为优先参考；输入信息很少时可以略短，但不要改成默认三段式。"
    else:
        style_rule = """
...
- 老师输入里是否提到了作业完成度：如是否完成、完成多少、是否拖拉、是否需要提醒。
- 老师输入里是否提到了作业质量：如正确率、书写、步骤、计算、订正、错题情况。
- 老师输入里是否提到了晚辅状态：如专注度、效率、主动提问、独立完成、依赖提醒。
- 老师输入里是否提到了主要问题：如薄弱题型、粗心、审题、计算、理解、时间管理。
- 老师输入里是否提到了后续提醒：如订正、复盘、整理错题、提高效率、保持习惯。
...
4. 语气自然、具体、克制，适合发给家长。
5. 优先覆盖老师已经提供的信息；可以把零散表述整理得更顺，但不能新增题目、错题数量、正确率、学习态度或家长行动。
