...
async def generate_evening_feedback(
    ...
    style_rule = (
        "12. 已提供老师的晚辅反馈风格样例；只学习晚辅反馈的称呼方式、分段习惯、段落长短、语气强弱、提醒方式和自然结构，禁止复用样例里的学生事实、原句、题目、时间或具体表现。"
        if style_text
        else "12. 老师暂未提供晚辅反馈风格样例，本次按自然、清楚、克制的晚辅反馈写法生成。"
    )
    prompt = f"""
...
8. 不强制固定标题格式，也不要强行套四段标题；可按老师样例或输入信息自然分段，正文要清楚、方便老师修改。
9. 后续提醒只能来自老师输入或由输入直接推出的通用提醒，例如“订正错题”“复盘出错原因”“提高完成效率”；不要新增具体作业任务。
10. 称呼学生时使用“{student_name}”，不要补全或猜测其他姓名。
11. {period_rule}
{style_rule}
